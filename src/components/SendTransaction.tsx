...
export const SendTransaction = ({ onSendTransaction, sender }: ISendTransaction) => {
  ...
  const handleButton = () => {
    ...
    onSendTransaction(initialValue)
      .then((hash) => {
    ...
    <div className='send-transaction'>
      ...
      {!transactionHash ? (
        <div className='codeBlock'>
          <pre className='code'>{jsonPayload}</pre>
        </div>
      ) : (
      ...
      <div className='divider' />
      <button className='w-button send-transaction__button' onClick={handleButton}>
        {showBackButton ? 'Back' : 'Send'}
      </button>
    </div>
  );
};
